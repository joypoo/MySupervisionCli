...
void CPropertiesWnd::AdjustLayout()
{
	...
	//int cyTlb = 0;//2024-3-27，屏蔽工具条 = m_wndToolBar.CalcFixedLayout(FALSE, TRUE).cy;
	int cyTlb = m_wndToolBar.CalcFixedLayout(FALSE, TRUE).cy;

	m_wndObjectCombo.SetWindowPos(nullptr, rectClient.left, rectClient.top, rectClient.Width(), m_nComboHeight, SWP_NOACTIVATE | SWP_NOZORDER);
#ifdef _MBCS
	m_wndObjectCombo.MoveWindow(rectClient.left, rectClient.top, rectClient.Width(), m_nComboHeight * 3);
#endif // _MBCS
...
int CPropertiesWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndObjectCombo.AddString(_T("地图"));
	m_wndObjectCombo.AddString(_T("现场"));
	...
	//m_wndToolBar.SetPermament(FALSE);
...
void CPropertiesWnd::InitPropList()
{
	...
	*/
	double fLong = 120.123456, fLat = 30.567890;
	CMFCPropertyGridProperty* pSize = new CMFCPropertyGridProperty(_T("位置信息"), 0, TRUE);

	CMFCPropertyGridProperty* pProp = new CMFCPropertyGridProperty(_T("经度"), (_variant_t)fLong, _T("位置经度"));
	//pProp->EnableSpinControl(TRUE, 50, 300);
	pSize->AddSubItem(pProp);
	pProp->AllowEdit(FALSE);

	pProp = new CMFCPropertyGridProperty( _T("纬度"), (_variant_t)fLat, _T("位置纬度"));
